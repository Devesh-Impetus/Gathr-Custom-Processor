	...
	private static Log logger = LogFactory.getLog(SaltGenerator.class);
		...
		public static void main(String[] args) {
			...
			for(int i=0;i<alpha.length;i++){
				logger.debug("Salt for "+alpha[i]+" : "+getSalt(alpha[i],12));
			}
			...
			//Test3		
			for(int i=0;i<alpha.length;i++){
				logger.debug("Salt for "+alpha[i]+" : "+getSalt(alpha[i]+i,12));
			}
			//test 4
			logger.debug("Test4 : " + getSalt("dakkjdasjdladjd;asdad;lkasd;lad;A;DKA';LD;'DASD65SA564D54SD4GSD54G5Sn4G54DSG54DS5G45DS4G5D4G4",12));
			//test 5
			logger.debug("Test5 : " + getSalt("3232248426",12));
		}
	}
